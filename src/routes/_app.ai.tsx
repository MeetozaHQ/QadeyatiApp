...
function AIPage() {
  ...
  const [lawyerMessages, setLawyerMessages] = useState<Record<string, ChatMessage[]>>({
    "1": [
      { role: "user", content: "كيف يمكنني صياغة بند التحكيم في عقد تجاري؟" },
      {
        role: "assistant",
        content:
          "لصياغة بند التحكيم بشكل سوي وقابل للتنفيذ في القانون المصري، يفضل كتابة الصياغة التالية:\n\n'ينشأ عن تنفيذ أو تفسير هذا العقد أي نزاع يتم تسويته نهائياً عن طريق التحكيم وفقًا لقواعد مركز القاهرة الإقليمي للتحكيم التجاري الدولي...'",
      },
    ],
    "2": [
      { role: "user", content: "ما هي شروط قبول دعوى الإخلاء لعدم سداد الإيجار؟" },
      {
        role: "assistant",
        content:
          "شروط قبول دعوى الإخلاء لعدم سداد الإيجار وفقاً لقوانين إيجار الأماكن المصرية:\n\n١. إعذار المستأجر بإنذار رسمي على يد محضر بسداد الأجرة المتأخرة.\n٢. انقضاء مدة ١٥ يوماً من تاريخ تسلم الإنذار دون السداد.\n٣. تقديم ما يفيد العلاقة الإيجارية والتقاعس عن الدفع.",
      },
    ],
  });

  const activeMessages =
    selectedLawyerId === "me" ? messages : lawyerMessages[selectedLawyerId] || [];

  const updateActiveMessages = (nextMsgs: ChatMessage[]) => {
    if (selectedLawyerId === "me") {
      setMessages(nextMsgs);
    } else {
      setLawyerMessages((prev) => ({ ...prev, [selectedLawyerId]: nextMsgs }));
    }
  };
  ...
  const send = async (text: string) => {
    ...
    if (selectedLawyerId !== "me") {
      ...
      if (lawyerUsage >= 600) {
        setError(
          `تنبيه: لقد استهلك المحامي (${lawyer?.name || "المكلف"}) كامل الحد الأقصى المسموح به للمساعد الذكي في باقة المكاتب والشركات (600 طلب شهرياً). سيتم تصفير العداد تلقائياً مع بداية الشهر الميلادي الجديد للمشترك.`,
        );
        return;
      }
    } else {
      if (aiCount >= limits.maxAIChats) {
        setError(
          `لقد استهلكت كامل حدك للمستشار الذكي لهذه الباقة (${limits.maxAIChats} طلب شهرياً). يرجى الترقية إلى باقة أعلى للتمتع بحد ممتد.`,
        );
        return;
      }
    }
    ...
    setError(null);
    const next: ChatMessage[] = [...activeMessages, { role: "user", content }];
  ...
  return (
    <div className="-mx-4 -mt-4 flex h-[calc(100dvh-3.5rem-5rem)] flex-col">
      <div className="border-b border-border bg-background/80 px-4 py-3 backdrop-blur-md flex items-center justify-between">
        <div>
        ...
        <div className="text-left">
          <span className="rounded-full bg-slate-900 border border-slate-800/40 px-2.5 py-1 text-[11px] font-medium text-slate-300 font-sans">
            الاستخدام:{" "}
            <span className="text-[var(--gold-soft)] font-bold">
              {selectedLawyerId === "me"
                ? aiCount
                : (firmLawyers.find((l) => l.id === selectedLawyerId)?.aiUsage ?? 0)}
            </span>{" "}
            / {limits.maxAIChats === Infinity ? "∞" : limits.maxAIChats}
          </span>
        </div>
      </div>
      ...
      {plan === "enterprise" && simulatedLawyerId === "owner" && (
        <div className="bg-[#0b0f19] border-b border-border px-4 py-2 flex items-center gap-2 overflow-x-auto text-right">
          ...
          <div className="flex gap-1.5 overflow-x-auto">
            <button
              onClick={() => setSelectedLawyerId("me")}
              className={cn(
                "rounded-lg px-2.5 py-1 text-xs font-semibold whitespace-nowrap transition-colors",
                selectedLawyerId === "me"
        ...
        {activeMessages.length === 0 && (
          <div className="space-y-4">
            <p className="text-center text-sm text-muted-foreground">
              اختر قالبًا أو اكتب سؤالك القانوني لـ{" "}
              {selectedLawyerId === "me"
                ? "حسابك الرئيسي"
                : firmLawyers.find((l) => l.id === selectedLawyerId)?.name || "المحامي المكلف"}
              .
            </p>
