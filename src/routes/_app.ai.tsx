...
function AIPage() {
  ...
  const send = async (text: string) => {
    ...
    if (selectedLawyerId !== "me") {
      ...
      if (lawyerUsage >= 400) {
        setError(
          `تنبيه: لقد استهلك المحامي (${lawyer?.name || "المكلف"}) كامل الحد الأقصى المسموح به للمساعد الذكي في باقة المكاتب والشركات (400 طلب شهرياً). سيتم تصفير العداد تلقائياً مع بداية الشهر الميلادي الجديد للمشترك.`,
        );
        return;
      }
    } else {
